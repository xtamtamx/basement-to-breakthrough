...
export const CardTransformationModal: React.FC<CardTransformationModalProps> = ({
  card,
  transformation,
  onConfirm,
  onCancel
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">{getTransformTypeIcon(transformation.result.type)}</span>
                <div>
                  <h3 className="pixel-text pixel-text-sm" style={{ color: getRarityColor(transformation.rarity) }}>
                    {transformation.name}
                  </h3>
                  <p className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-gray)' }}>
                    {transformation.rarity.toUpperCase()} {transformation.result.type.toUpperCase()}
                  </p>
                </div>
              </div>
              
              <p className="pixel-text pixel-text-xs mb-3" style={{ color: 'var(--pixel-white)' }}>
              ...
              <div className="space-y-2">
                {transformation.result.modifications.stats && (
                  <div>
                    <p className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-yellow)' }}>
                      STAT CHANGES:
                    </p>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {Object.entries(transformation.result.modifications.stats).map(([stat, value]) => (
                        <span 
                          key={stat}
                          className="pixel-badge"
                          style={{ 
                            backgroundColor: value > 0 ? 'var(--pixel-green)' : 'var(--pixel-red)',
                            fontSize: '6px'
                          }}
                        >
                ...
                {transformation.result.modifications.addTraits && (
                  <div>
                    <p className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-cyan)' }}>
                      NEW TRAITS:
                    </p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {transformation.result.modifications.addTraits.map(trait => (
                        <span 
                          key={trait}
                          className="pixel-badge"
            ...
            {transformation.result.type === 'corrupt' && (
              ...
              >
                <p className="pixel-text pixel-text-xs text-center" style={{ color: 'var(--pixel-red)' }}>
            ...
            <div className="flex gap-3">
              <button
                ...
                style={{
                  background: confirmed ? 'var(--pixel-gray)' : 
