    ...
    <div className="city-district-wrapper">
      ...
      {venues
        .filter(venue => {
          ...
          return venue.gridPosition.x >= gridBounds.x &&
        ...
        .map(venue => {
          ...
          return (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => handleCellClick(venue.gridPosition!.x, venue.gridPosition!.y)}
              title={venue.name}
            >
            <div className="venue-content">
              <div className="venue-icon">
                ...
                {venue.type === 'theater' && '🎭'}
                {venue.type === 'garage' && '🚗'}
                {venue.type === 'diy_space' && '🎨'}
              </div>
              <div className="venue-name">{venue.name}</div>
            </div>
            {venue.upgrades && venue.upgrades.length > 0 && (
              <div className="venue-upgrades">
                <span className="upgrade-count">{venue.upgrades.length}</span>
              </div>
            )}
            {/* Show indicator */}
      ...
      >
        {Array.from({ length: gridBounds.width * gridBounds.height }).map((_, index) => {
          ...
          return (
            <div
              ...
              onMouseEnter={() => setHoveredCell({ x: globalX, y: globalY })}
