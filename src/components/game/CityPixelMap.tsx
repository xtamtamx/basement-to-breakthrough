  ...
  return (
    <div className="relative w-full h-full bg-gray-900 rounded-lg overflow-hidden">
      ...
      <div className="relative h-full flex flex-col p-4">
        ...
        {/* City Grid */}
        <div
          className={`flex-1 relative grid ${districtId ? "grid-cols-5 grid-rows-4" : "grid-cols-10 grid-rows-8"} gap-1 p-4 bg-gray-800 rounded-lg`}
        >
          {/* Grid lines */}
          {Array.from({ length: districtId ? 20 : 80 }).map((_, i) => {
            ...
            const hasDistrict = displayDistricts.some(
              (d) => d.x === x && d.y === y,
            );
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                ...
                >
                  <BaseSprite
                    data={
                      BUILDING_SPRITES[
                        district.sprite as keyof typeof BUILDING_SPRITES
                      ]
                    }
                    pixelSize={4}
                    colorMap={
                      colorMaps[district.sprite as keyof typeof colorMaps]
                    }
                  />
                </svg>
              </div>
          ...
          {districtId && (
            <>
              ...
              {venues
                .filter((v) => v.location?.id === districtId)
                .map((venue, i) => (
                  <motion.div
                    key={venue.id}
                    className="absolute cursor-pointer"
        ...
        {hoveredDistrict && (
          ...
          >
            {(() => {
              const district = displayDistricts.find(
                (d) => d.id === hoveredDistrict,
              );
