...
export const DistrictDetailView: React.FC<DistrictDetailViewProps> = ({
  ...
  console.log('DistrictDetailView - districtId:', districtId, 'district:', district);
  if (!district) {
    console.error('District not found for ID:', districtId);
    return <div className="w-full h-full flex items-center justify-center text-white">District not found: {districtId}</div>;
  }
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] bg-gray-900 rounded-lg overflow-hidden">
      ...
      <div className="absolute inset-0" style={{ backgroundColor: district.color + '10' }}>
      ...
      <div className="relative h-full flex flex-col p-4">
        ...
        <div className="flex-1 grid grid-cols-4 grid-rows-3 gap-4 p-4 bg-gray-800 bg-opacity-50 rounded-lg">
          ...
          >
            ...
            <div className="text-center mt-2">
              <span className="text-sm font-bold text-white">{district.name} Hub</span>
            </div>
          </motion.div>

          {/* Job Locations */}
          {district.jobs.map((job, i) => (
            <motion.div
              key={`job-${i}`}
          ...
          {Array.from({ length: Math.max(0, 2 - districtVenues.length) }).map((_, i) => (
            <motion.div
              key={`empty-${i}`}
              className="relative"
              initial={{ opacity: 0 }}
        ...
        <div className="mt-4 flex justify-center gap-8">
          <div className="text-center">
            ...
            <div className="text-lg font-bold text-white">{districtVenues.length}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-400">Jobs</div>
            <div className="text-lg font-bold text-green-400">{district.jobs.length}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-400">Activity</div>
            <div className="text-lg font-bold" style={{ color: district.color }}>High</div>
          </div>
        </div>
      </div>
