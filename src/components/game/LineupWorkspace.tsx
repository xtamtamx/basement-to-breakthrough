...
export const LineupWorkspace = forwardRef<HTMLDivElement, LineupWorkspaceProps>(
  ({ lineupStacks, onDrop, onBandDragStart, onStackUpdate, draggedBand }, ref) => {
    ...
    const removeFromStack = (stackId: string, bandId: string) => {
      const updatedStacks = lineupStacks.map(stack => {
        if (stack.id === stackId) {
          return {
            ...stack,
    ...
    return (
      <div
        ...
        onDrop={handleDrop}
      >
        <h2 className="pixel-text pixel-text-sm mb-4 punk-neon-cyan" style={{ color: 'var(--punk-neon-cyan)' }}>
          SHOW BUILDER
        </h2>
        ...
        {lineupStacks.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center opacity-20">
              <p className="pixel-text pixel-text-lg mb-2 punk-stencil" style={{ color: 'var(--punk-neon-purple)' }}>
                DROP BANDS HERE
              </p>
        ...
        <AnimatePresence>
          {lineupStacks.map((stack) => {
            ...
            const bill = stack.bands.length >= 2 ? billManager.analyzeBill(stack.bands) : null;
            ...
            return (
              <motion.div
                ...
                onDragStart={(e) => {
                  ...
                  e.dataTransfer.setData('lineup', JSON.stringify(stack.bands));
                }}
              >
                {/* Stack Header */}
                  ...
                  {bill && (
                    <div className="mt-2 grid grid-cols-2 gap-1">
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--pixel-green)' }} />
                        <span className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-gray)' }}>
                          CHEM: {bill.dynamics.chemistryScore}%
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--pixel-yellow)' }} />
                        <span className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-gray)' }}>
                    ...
                    <div className="space-y-1">
                      {synergies.slice(0, 3).map((synergy, i) => (
                        <div key={i} className="flex items-center gap-1">
                          ...
                          <span className="pixel-text" style={{ fontSize: '8px', color: 'var(--pixel-cyan)' }}>
                            {synergy.name}
                          </span>
                        </div>
                      ))}
                ...
                <div className="mt-3 text-center">
                  <p className="pixel-text pixel-text-xs animate-pulse" style={{ color: 'var(--pixel-yellow)' }}>
                    DRAG TO VENUE →
                  </p>
                </div>
              </motion.div>
