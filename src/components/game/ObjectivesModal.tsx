...
export const ObjectivesModal: React.FC<ObjectivesModalProps> = ({ onClose }) => {
  ...
  return (
    <SnesModal onClose={onClose} title="🎯 Challenges" ariaLabel="Challenges" maxWidth={460} accent="var(--snes-purple)">
      <div>
              ...
              return (
                <div key={p.id} className="snes-panel-inset" style={{ padding: '12px', opacity: p.completed ? 1 : 0.96 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '10px', marginBottom: '6px' }}>
                    <h3 className="snes-pixel" style={{ fontSize: '9px', color: p.completed ? 'var(--snes-green)' : 'var(--snes-ink)', margin: 0, letterSpacing: 0, lineHeight: 1.4 }}>
                      {p.completed ? '✓ ' : ''}{def.title}
                    </h3>
                    <span className="snes-pixel" style={{ fontSize: '8px', color: 'var(--snes-magenta)', letterSpacing: 0, flexShrink: 0 }}>
                      +{def.fameReward}
                    </span>
                  </div>
                  ...
                  {!def.finalizeOnly && !p.completed && (
                    <div style={{ fontSize: '10px', color: 'var(--snes-ink-mute)', marginTop: '4px', textAlign: 'right' }}>
                      {p.current}/{p.target}
                    </div>
                  )}
                </div>
