...
export const CityMapCanvas: React.FC<CityMapCanvasProps> = ({
  ...
  const renderTile = useCallback(
    (ctx: CanvasRenderingContext2D, tile: MapTile, time: number) => {
      ...
      if (
        ...
        screenPos.x > width + tileSize ||
        screenPos.y < -tileSize ||
        screenPos.y > height + tileSize
      ) {
        return;
      }
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const worldX = Math.floor(
        ((clickX - width / 2) / camera.zoom + camera.x) / map.tileSize,
      );
      const worldY = Math.floor(
        ((clickY - height / 2) / camera.zoom + camera.y) / map.tileSize,
      );
  ...
  return (
    ...
    >
      ...
      >
        Camera: ({Math.floor(camera.x)}, {Math.floor(camera.y)}) Zoom:{" "}
