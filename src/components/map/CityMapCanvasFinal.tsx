...
import { SPRITES, scaleSprite, getSpriteForTile, SpriteData } from './sprites/PixelSprites';
...
export const CityMapCanvasFinal: React.FC = () => {
      ...
      if (spriteData) {
        ...
        for (let y = 0; y < spriteSize; y++) {
          for (let x = 0; x < spriteSize; x++) {
            ...
            if (spriteData.data[idx + 3] > 0) { // Has alpha
  ...
  }, [cityMap, camera.zoom, worldToScreen, dimensions, selectedTile, spriteCache]);
  ...
  const render = useCallback((time: number) => {
    ...
    if (camera.zoom > 1) {
      ...
      const offsetX = (dimensions.width / 2 - camera.x * camera.zoom) % gridSize;
      const offsetY = (dimensions.height / 2 - camera.y * camera.zoom) % gridSize;
    ...
    if (playerPosition) {
      ...
      ctx.beginPath();
      ctx.ellipse(screenPos.x, screenPos.y + 4, 10 * camera.zoom, 6 * camera.zoom, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      for (let y = 0; y < cityMap.height; y++) {
        for (let x = 0; x < cityMap.width; x++) {
          ...
          if (tile.type === 'venue' && (tile.data as VenueData)?.hasActiveShow) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText('LIVE', screenPos.x, screenPos.y - 10 * camera.zoom + bounce);
          }
        }
      }
  ...
  }, [cityMap, screenToWorld, setSelectedTile, spawnVenueParticles, handleTileClick]);
    ...
    return (
      <div style={{
        ...
        <div>
          ...
          <div style={{ width: '200px', height: '4px', backgroundColor: '#333333', position: 'relative' }}>
            <div style={{
              position: 'absolute',
              left: 0,
              top: 0,
