...
import { clampCamera, getDistanceBetweenPoints, getMidpoint } from '@/utils/cameraUtils';
import { haptics } from '@/utils/mobile';
import { useMapInteraction } from '@/hooks/useMapInteraction';
import { SPRITES_16BIT, SPRITE_COLORS_16BIT } from './sprites/PixelSprites16Bit';
...
export const CityMapPixelArt: React.FC = () => {
  ...
  const getCameraConstraints = useCallback(() => {
    if (!cityMap) return { minX: 320, maxX: 320, minY: 240, maxY: 240, minZoom: 0.5, maxZoom: 2 };
  ...
  const drawPixelArtTile = useCallback((
    ...
    if (tile.type === 'building' && tile.district) {
      ...
      sprite = tile.animated || (tile.data as VenueData)?.hasActiveShow 
      ...
      if (!cityMap) {
        ...
        const hasNorth = tile.y > 0 && cityMap.tiles[tile.y - 1][tile.x].type === 'street';
        const hasSouth = tile.y < cityMap.height - 1 && cityMap.tiles[tile.y + 1][tile.x].type === 'street';
        const hasEast = tile.x < cityMap.width - 1 && cityMap.tiles[tile.y][tile.x + 1].type === 'street';
        const hasWest = tile.x > 0 && cityMap.tiles[tile.y][tile.x - 1].type === 'street';
    ...
    if (sprite) {
      ...
      if (tile.type === 'venue' && (tile.animated || (tile.data as VenueData)?.hasActiveShow)) {
        ctx.save();
  ...
  const render = useCallback((time: number) => {
    ...
    animationFrameRef.current = requestAnimationFrame(render);
  }, [cityMap, camera, dimensions, getCameraConstraints, setCamera, worldToScreen, drawPixelArtTile]);
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    } else if (e.touches.length === 1 && isDragging && touchesRef.current.size >= 1) {
      // Handle pan
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    if (!isDragging && e.changedTouches.length === 1 && touchesRef.current.size === 1) {
  ...
  return (
    <div ref={containerRef} className="city-map-container" style={{
      width: '100%',
      height: '100%',
      position: 'relative',
