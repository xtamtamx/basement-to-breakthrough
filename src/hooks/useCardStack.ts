...
export const useCardStack = (
  ...
  const { stackThreshold = 50, stackOffset = 5, maxStackSize = 5 } = options;
  ...
  const getStackCards = useCallback(
    (stackId: string): CardPosition[] => {
      return positions
        .filter((pos) => pos.stackId === stackId)
  ...
  const findNearestStackable = useCallback(
    (
      ...
      stacks.forEach((stack) => {
        const distance = Math.sqrt(
          Math.pow(position.x - stack.x, 2) + Math.pow(position.y - stack.y, 2),
        );
  ...
  const updatePosition = useCallback(
    (cardId: string, newPosition: { x: number; y: number }) => {
      setPositions((prev) =>
        prev.map((pos) =>
          pos.id === cardId
  ...
  const handleCardDrop = useCallback(
    (cardId: string, dropPosition: { x: number; y: number }) => {
  ...
  const fanStack = useCallback(
    (stackId: string, fanDistance: number = 20) => {
      const stack = stacks.get(stackId);
      if (!stack) return;
